...
Usage (run from pi/):
    python ./infer/async.py --checkpoint ./ckpt/PIv3 --task "Pick up the red lego." --duration 60

Hardware Setup:
    - Robot: SO-101 follower arm on /dev/ttyACM0
    - Cameras: main (/dev/video4), secondary_0 (/dev/video2), secondary_1 (/dev/video0)
      - Use lerobot-find-cameras to discover which /dev/video* path corresponds to which physical camera, then map them to the
        correct key names accordingly.
"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to checkpoint directory (e.g., ./ckpt/PIv3)",
    )
